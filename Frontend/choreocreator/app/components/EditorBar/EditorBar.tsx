...
type EditorBarProps = {
    ...
    onChangeTotalDuration: (ms: number) => void;
    onUpdateFormationMeta: (id: string, name: string, description: string) => void;
};
...
const EditorBar: React.FC<EditorBarProps> = (props) => {
    const {
        ...
        onUpdateFormationMeta
    } = props;

    const [selectedMenuKey, setSelectedMenuKey] = useState('1');
    const { mode } = useTheme();
    const selectedFormation = useMemo(() => formations.find(
        f => f.id === selectedFormationId)
        ?? null, [formations, selectedFormationId]);
    return <Sider theme={mode === 'dark' ? 'dark' : 'light'}
        width={250}
        style={{
            position: 'fixed',
            top: 65,
        ...
        {selectedMenuKey === '1' &&
            <>
                ...
                <div style={{ overflowY: 'auto', paddingBottom: 5 }}>
                    {dancerPositions.map((d, i) =>
                        <div
                            key={d.id}
                            onClick={() => onSelectDancer(d.id)}
                            style={{
                                padding: 8,
                                background: d.id === selectedDancerId ? 'var(--editor-item-selected-bg)' : 'transparent',
                                color: 'var(--editor-item-text)',
                                cursor: 'pointer',
                                display: 'flex',
                                alignItems: 'center',
                                gap: 8,
                                border: d.id === selectedDancerId ? '1px solid var(--editor-item-selected-border)' : '1px solid transparent'
                            }}
                        >
                            <span style={{
                                width: 14,
                                height: 14,
                                borderRadius: '50%',
                                background: 'var(--scene-dancer-fill, #cc3f7c)',
                                border: d.id === selectedDancerId ? '2px solid var(--scene-dancer-selected-stroke, #ff76b5)' : '1px solid var(--scene-dancer-stroke, #8f2554)',
                                display: 'inline-block'
                            }}
                            />
                            Танцор {i + 1} ({d.position.x}, {d.position.y})
                        </div>)
                    }
                </div>
                <div style={{
        ...
        {selectedMenuKey === '2' && <>
            <div style={{ padding: 10 }}>
                ...
                <InputNumber
                    min={10}
                    max={1200}
                    value={Math.round(totalDurationMs / 1000)}
                    onChange={(v) => onChangeTotalDuration((Number(v) || 10) * 1000)}
            ...
            >
                {formations.map((f, i) =>
                    <div key={f.id}
                        ...
                        style={{
                            padding: 8,
                            background: f.id === selectedFormationId ? 'var(--editor-item-selected-bg)' : 'transparent',
                            color: 'var(--editor-item-text)',
                            cursor: 'pointer'
                        }}
                    >
            ...
            {selectedFormation && <div style={{ padding: 10 }}>
                <Input
                    ...
                    placeholder="Имя"
                    onChange={(e) => onUpdateFormationMeta(selectedFormation.id, e.target.value, selectedFormation.description)}
                    style={{ marginBottom: 8 }} />
                <Input.TextArea
                    value={selectedFormation.description}
                    placeholder="Описание"
                    rows={3}
                    onChange={(e) => onUpdateFormationMeta(selectedFormation.id, selectedFormation.name, e.target.value)} />
            </div>}
            <div style={{
                position: 'absolute',
                bottom: 10,
